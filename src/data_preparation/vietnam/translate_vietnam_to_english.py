...
def translate_sentence(src_sentence: str, client: translate.TranslationServiceClient):
    response = client.translate_text(
        contents=[src_sentence], source_language_code="vi", target_language_code="en", parent=os.environ["PROJECT_ID"]
    )

    return response.translations[0].translated_text


def translate_queries(queries: list[dict], client: translate.TranslationServiceClient) -> list[dict]:
    translated = []
    for item in tqdm(queries, desc="Translating queries..."):
        ...
        translated.append({"query": query_translated, "articles": articles})

    return translated


def translate_articles(articles: list[dict], client: translate.TranslationServiceClient) -> list[dict]:
    translated = []
